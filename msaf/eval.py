...
def compute_gt_results(est_file, ref_file, boundaries_id, labels_id, config,
	...
	try:
		...
		if config["hier"]:
			...
			ref_inter, ref_labels = jams2.converters.load_jams_range(ref_file, "sections", annotator=annotator_id, context=context)
...
def process(in_path, boundaries_id, labels_id=None, ds_name="*",
			...
			save=False, n_jobs=4, annotator_id=0, config=None, plot=False):
	"""Main process.
