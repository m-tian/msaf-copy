...
def get_dataset_files(in_path, ds_name="*"):
	...
	'''Mi: added new Jingju dataset CJ'''
	...
	# Get audio files
	audio_files = []
	for ext in msaf.Dataset.audio_exts: # .wav, .mp3, .aif
		# The following line doesn't work for audio names containing "_".
		# audio_files += glob.glob(os.path.join(in_path, msaf.Dataset.audio_dir, ("%s_*" + ext) % prefix))
		audio_files += glob.glob(os.path.join(in_path, msaf.Dataset.audio_dir, ("%s_*" + ext) % prefix))
