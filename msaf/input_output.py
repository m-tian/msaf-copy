...
def get_dataset_files(in_path, ds_name="*"):
	...
	audio_files = [x for x in os.listdir(os.path.join(in_path, msaf.Dataset.audio_dir)) if (not x.startswith('.') and (x.endswith('.aif') or \
	x.endswith('.wav') or x.endswith('.mp3')) )]
	audio_files = [os.path.join(in_path, msaf.Dataset.audio_dir, x) for x in audio_files]
