...
//    cout << endl;
	...
	//////////////mi: Create new python lists of intermediate results/////////////
	int times_temp_len = segmentation.segments_temp.size() + 1;
    PyObject *times_temp = PyList_New(times_temp_len);
    if (!times_temp)
        return NULL;
    PyObject *num_temp = PyInt_FromLong(segmentation.segments_temp[0].start);
    PyList_SET_ITEM(times_temp, 0, num_temp);
    for (int i = 0; i < times_temp_len - 1; i++) {
        PyObject *num_temp = PyInt_FromLong(segmentation.segments_temp[i].end);
        if (!num_temp) {
            Py_DECREF(times_temp);
            return NULL;
        }
        PyList_SET_ITEM(times_temp, i + 1, num_temp);
    }

    // Put Labels in a Python list
    int labels_temp_len = segmentation.segments_temp.size();
    PyObject *labels_temp = PyList_New(labels_temp_len);
    if (!labels_temp)
        return NULL;
    for (int i = 0; i < labels_temp_len; i++) {
        PyObject *num_temp = PyInt_FromLong(segmentation.segments_temp[i].type);
        if (!num_temp) {
            Py_DECREF(labels_temp);
            return NULL;
        }
        PyList_SET_ITEM(labels_temp, i, num_temp);
    }
	////////////////////////////////mi: Ends here////////////////////////////////
	...
	/*mi: Return 2 segments instances, one final the other original hmm results*/
	return Py_BuildValue("[O,O,O,O]", times, labels, times_temp, labels_temp);
