...
*/
	...
	// mi: Save intermediate cluster assignments
	temp_q = new int[features.size()];
	
    // uri: Do everything here. Chroma features are already normalised
    cluster_segment(q, temp_q, arrFeatures, features.size(), features[0].size(), nHMMStates, histogramLength, 
...
void ClusterMeltSegmenter::makeSegmentation(int* q, int* temp_q, int len)
{
    ...
    segment_temp.start = 0;
    segment_temp.type = q[0];
    ...
    if (annotBounds.size() == 0) {
		...
		for (int i = 1; i < len; i++)
        {
            if (temp_q[i] != temp_q[i-1])
            {
                segment_temp.end = i;
                segmentation.segments.push_back(segment_temp);
                segment_temp.type = temp_q[i];
                segment_temp.start = segment_temp.end;
            }
        }
        // segment.end = len * getHopsize();
        segment_temp.end = len-1;
        segmentation.segments.push_back(segment_temp);
	...
	segmentation.segments.push_back(q);
	segmentation.segments.push_back(temp_q);
